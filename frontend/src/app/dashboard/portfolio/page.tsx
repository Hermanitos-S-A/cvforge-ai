...
import { Globe, Download, ExternalLink, Loader2, CheckCircle, Rocket } from "lucide-react";
...
export default function PortfolioPage() {
  ...
  const downloadHTML = async () => {
    ...
    try {
      ...
      toast.success("Portfolio HTML descargado!");
    } catch {
      toast.error("Error al generar portafolio — verifica que el backend esta corriendo");
    } finally { setDownloading(false); }
  };

  const sections = [
    "Hero con bio profesional",
    "Grid de habilidades",
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Web personal generada automaticamente desde tu CV</p>
        </div>
      </div>
            ...
            <div className="text-white/90 text-xs">
              <div className="text-center py-8 px-6 border-b border-white/10 bg-gradient-to-b from-[#16161f] to-[#0a0a0f]">
                ...
                <p className="text-white/40 text-xs max-w-sm mx-auto leading-relaxed">
                  {resume.summary || "Desarrollador apasionado construyendo aplicaciones web escalables."}
                </p>
              ...
              <div className="grid grid-cols-3 divide-x divide-white/10">
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(resume.skills.length > 0 ? resume.skills.slice(0, 6) : [{ name: "React" }, { name: "TypeScript" }, { name: "Python" }, { name: "Docker" }]).map((s: any) => (
                ...
                <div className="p-4">
                  ...
                  {(resume.projects.length > 0 ? resume.projects.slice(0, 2) : [{ name: "Mi Proyecto", description: "App web full-stack" }]).map((pr: any, i: number) => (
                    <div key={i} className="mb-2 bg-white/5 rounded-lg p-2">
                      <div className="font-medium text-[11px] text-white/80">{pr.name}</div>
                      <div className="text-[10px] text-white/40 mt-0.5">{pr.description}</div>
                    </div>
                  ))}
                </div>
                <div className="p-4">
                  <div className="text-[9px] font-bold text-violet-400 uppercase tracking-widest mb-3">Experiencia</div>
                  {(resume.experiences.length > 0 ? resume.experiences.slice(0, 2) : [{ role: "Developer", company: "Empresa", start_date: "2022", is_current: true }]).map((e: any, i: number) => (
                    <div key={i} className="mb-2">
                      <div className="font-medium text-[11px] text-white/80">{e.role}</div>
                      <div className="text-[10px] text-violet-400">{e.company}</div>
                      <div className="text-[10px] text-white/30">{e.start_date} - {e.is_current ? "Presente" : e.end_date}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-4">
          <div className="p-5 rounded-2xl border border-border bg-card">
            ...
            <div className="space-y-3">
              <button onClick={() => toast.info("Para deploy en Vercel:\n1. Descarga el HTML\n2. Sube a un repo GitHub\n3. Conecta en vercel.com — gratis!")}
                ...
                <div className="text-left flex-1">
                  ...
                  <div className="text-xs text-muted-foreground font-normal">Hosting gratis, deploy instantaneo</div>
                </div>
                <ExternalLink size={11} className="text-muted-foreground group-hover:text-primary" />
              </button>
              <button onClick={downloadHTML} disabled={downloading}
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-muted-foreground font-normal">Archivo unico listo para usar</div>
                </div>
              </button>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {sections.map(s => (
                <div key={s} className="flex items-center gap-2 text-sm text-muted-foreground">
                  <CheckCircle size={12} className="text-emerald-400 flex-shrink-0" />{s}
                </div>
              ))}
            </div>
          </div>
