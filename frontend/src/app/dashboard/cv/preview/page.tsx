...
export default function CVPreviewPage() {
  ...
  const exportPDF = async () => {
    if (!serverResumeId) return toast.error("Guarda tu CV primero");
    setExporting(true);
    try {
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            className="bg-white rounded-2xl shadow-2xl overflow-hidden text-gray-800 text-[13px]"
            ...
            <div className="p-8 pb-6 text-white" style={{ background: tc.header }}>
              ...
              <p className="text-white/80 text-sm">{p.title || "Tu título profesional"}</p>
            ...
            <div className="p-8 space-y-5">
              ...
              {resume.educations.length > 0 && (
                <section>
                  <h2 className="text-[10px] font-bold uppercase tracking-widest mb-3 pb-1 border-b-2"
                    style={{ color: tc.accent, borderColor: tc.accent }}>Educación</h2>
                  <div className="space-y-2">
                    {resume.educations.map((edu: any, i: number) => (
                      <div key={i} className="flex justify-between">
                        <div>
                          <p className="font-bold text-[13px]">{edu.degree} {edu.field && `en ${edu.field}`}</p>
                    ...
                    {resume.projects.map((proj, i) => (
                      <div key={i}>
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-[13px]">{proj.name}</p>
                        ...
                        {proj.technologies?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {proj.technologies.map((t: string) => <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">{t}</span>)}
                          </div>
                        )}
                      </div>
                    ))}
              ...
              {!resume.summary && resume.experiences.length === 0 && resume.skills.length === 0 && (
                <div className="text-center py-16 text-gray-400">
                  <p className="text-sm mb-2">Tu CV aparecerá aquí mientras lo completas</p>
                  <Link href="/dashboard/cv" className="text-xs hover:underline" style={{ color: tc.accent }}>→ Ir al editor</Link>
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="p-5 rounded-2xl border border-border bg-card">
            <h3 className="font-semibold text-sm mb-4">Cambiar plantilla</h3>
            <div className="space-y-2">
              {[
                { id:"atlas",  label:"Atlas",  desc:"Header degradado · ATS ✓", color:"#6c63ff" },
                { id:"nova",   label:"Nova",   desc:"Tema azul limpio",          color:"#0ea5e9" },
                { id:"zenith", label:"Zenith", desc:"Dos columnas · ATS ✓",      color:"#10b981" },
              ].map((t) => (
                <Link key={t.id} href="/dashboard/templates"
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${resume.template === t.id ? "border-primary/40 bg-primary/8" : "border-border hover:border-border/60"}`}>
                  ...
                  {resume.template === t.id && <span className="text-xs text-primary font-bold">✓</span>}
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="p-5 rounded-2xl border border-border bg-card">
            <h3 className="font-semibold text-sm mb-4">Completitud</h3>
            <div className="space-y-2.5">
              {[
                { name:"Nombre y título",  done: !!(p.name && p.title)        },
                ...
                <div key={s.name} className="flex items-center gap-2 text-sm">
                  <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] flex-shrink-0 ${s.done ? "bg-emerald-500/20 text-emerald-400" : "bg-border text-muted-foreground"}`}>
                    {s.done ? "✓" : "·"}
                  </div>
          ...
          <div className="p-5 rounded-2xl border border-border bg-card">
            ...
            <div className="space-y-1.5">
              {[
                { href:"/dashboard/cv",        label:"Editar perfil",    icon:"✏" },
                { href:"/dashboard/ai",         label:"Optimizar con IA", icon:"✦" },
                { href:"/dashboard/ats",        label:"Analizar ATS",     icon:"◉" },
                { href:"/dashboard/portfolio",  label:"Ver portafolio",   icon:"◇" },
              ].map((a) => (
