...
class ApiClient {
  ...
  constructor() {
    this.http = axios.create({ baseURL: "/api/v1", headers: { "Content-Type": "application/json" }, timeout: 15000 });
    this.http.interceptors.request.use((config) => { const t = getToken(); if (t) config.headers.Authorization = "Bearer " + t; return config; });
    this.http.interceptors.response.use((r) => r, (err: AxiosError) => { if (err.response?.status === 401) clearAuth(); return Promise.reject(err); });
  }
  ...
  async createResume(payload: object) { const { data } = await this.http.post("/resumes", payload); return data; }
  async updateResume(id: number, payload: object) { const { data } = await this.http.patch("/resumes/" + id, payload); return data; }
  async deleteResume(id: number) { await this.http.delete("/resumes/" + id); }
  async addExperience(resumeId: number, payload: object) { const { data } = await this.http.post("/resumes/" + resumeId + "/experiences", payload); return data; }
  async deleteExperience(resumeId: number, expId: number) { await this.http.delete("/resumes/" + resumeId + "/experiences/" + expId); }
  async addEducation(resumeId: number, payload: object) { const { data } = await this.http.post("/resumes/" + resumeId + "/education", payload); return data; }
  async addSkill(resumeId: number, payload: object) { const { data } = await this.http.post("/resumes/" + resumeId + "/skills", payload); return data; }
  async deleteSkill(resumeId: number, skillId: number) { await this.http.delete("/resumes/" + resumeId + "/skills/" + skillId); }
  async addProject(resumeId: number, payload: object) { const { data } = await this.http.post("/resumes/" + resumeId + "/projects", payload); return data; }
  async deleteProject(resumeId: number, projId: number) { await this.http.delete("/resumes/" + resumeId + "/projects/" + projId); }
  async optimizeText(text: string, context = "experience") { const { data } = await this.http.post("/ai/optimize", { text, context }); return data; }
  async generateBio(platform: string, tone: string) { const { data } = await this.http.post("/ai/bio", { platform, tone }); return data; }
  ...
  async getAllPlans() { const { data } = await this.http.get("/plans/all"); return data; }
