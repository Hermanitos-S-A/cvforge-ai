...
app = FastAPI(
    ...
    description="AI-powered CV builder, ATS analyzer, and portfolio generator. 100% free & open source.",
    docs_url="/api/docs",
    redoc_url="/api/redoc",
)
...
app.include_router(ai.router,      prefix=f"{PREFIX}/ai",      tags=["AI Assistant"])
app.include_router(ats.router,     prefix=f"{PREFIX}/ats",     tags=["ATS Analyzer"])
...
@app.get("/", tags=["Root"])
def root():
    return {"message": "CVForge AI API", "docs": "/api/docs", "version": settings.APP_VERSION}
